...
/// @title Canonical SectaFi Dex factory
/// @notice Deploys SectaFi Dex pools and manages ownership and control over pool protocol fees
contract SectaDexFactory is ISectaDexFactory {
    /// @inheritdoc ISectaDexFactory
    ...
    function createPool(
        ...
        pool = ISectaDexPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ISectaDexPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
