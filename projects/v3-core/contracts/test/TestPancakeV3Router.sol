...
contract TestSectaDexRouter is ISectaDexSwapCallback {
    ...
    function sectaDexSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? ISectaDexPool(msg.sender).token0()
                : ISectaDexPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == ISectaDexPool(pools[0]).token1();
            ISectaDexPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(ISectaDexPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(ISectaDexPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
