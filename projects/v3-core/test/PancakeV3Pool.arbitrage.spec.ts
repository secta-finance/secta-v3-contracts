...
import { SectaDexPoolSwapTest } from '../typechain-types/contracts/test/SectaDexPoolSwapTest'
...
describe('SectaDexPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('SectaDexPoolSwapTest')
            const tester = (await testerFactory.deploy()) as SectaDexPoolSwapTest
          ...
          let tester: SectaDexPoolSwapTest
