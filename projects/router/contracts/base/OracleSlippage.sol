...
import '@sectafi/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@sectafi/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@sectafi/v3-periphery/contracts/libraries/Path.sol';
import '@sectafi/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@sectafi/v3-core/contracts/interfaces/ISectaDexPool.sol';
import '@sectafi/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ISectaDexPool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ISectaDexPool pool) {
        pool = ISectaDexPool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ISectaDexPool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ISectaDexPool(pool).slot0();
            }
