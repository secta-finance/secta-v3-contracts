...
import { IUniswapV2Pair, IWETH9, MockTimeSmartRouter, MixedRouteQuoterV1, TestERC20 } from '../typechain'
...
describe('SmartRouter', function () {
  ...
  const smartRouterFixture: Fixture<{
    weth9: IWETH9
    deployer: Contract
    factory: Contract
    factoryV2: Contract
    ...
    const { weth9, deployer, factory, factoryV2, router, helper, tokens, nft } = await completeFixture(
      wallets,
      provider
    )
    ...
    const quoterFactory = await ethers.getContractFactory('MixedRouteQuoterV1', {
      libraries: { SmartRouterHelper: helper.address },
    })
  ...
  let weth9: IWETH9
  let router: MockTimeSmartRouter
  let helper: Contract
  ...
  beforeEach('load fixture', async () => {
    ;({ router, helper, quoter, weth9, deployer, factory, factoryV2, tokens, nft } = await loadFixture(
      smartRouterFixture
    ))
      ...
      describe('multi-pool', () => {
        ...
        it('events', async () => {
          await expect(
            ...
            .withArgs(
              router.address,
              computePoolAddress(deployer.address, [tokens[1].address, tokens[2].address], FeeAmount.MEDIUM),
              3
            )
      ...
      describe('multi-pool', () => {
        ...
        it('events', async () => {
          await expect(
            ...
            .to.emit(tokens[2], 'Transfer')
            .withArgs(
              computePoolAddress(deployer.address, [tokens[2].address, tokens[1].address], FeeAmount.MEDIUM),
              trader.address,
              1
            )
            .to.emit(tokens[1], 'Transfer')
            .withArgs(
              computePoolAddress(deployer.address, [tokens[1].address, tokens[0].address], FeeAmount.MEDIUM),
              computePoolAddress(deployer.address, [tokens[2].address, tokens[1].address], FeeAmount.MEDIUM),
              3
            )
            .to.emit(tokens[0], 'Transfer')
  ...
  describe('swaps - v2 + v3', () => {
    ...
    describe('interleaving', () => {
      ...
      it('exactIn 0 -V3-> 1 -V2-> 2', async () => {
        ...
        const { amountOut: quoterAmountOut } = await quoter.callStatic['quoteExactInput(bytes,uint256[],uint256)'](
