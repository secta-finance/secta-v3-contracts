...
```solidity
function add(uint256 _allocPoint, ISectaDexPool _v3Pool, bool _withUpdate) external
```
...
| _v3Pool | ISectaDexPool | Address of the V3 pool. |
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, ISectaDexPool v3Pool, ILMPool lmPool)
```
