...
import { ISectaDexPool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [ISectaDexPool, ISectaDexPool, ISectaDexPool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, ISectaDexPoolABI, wallet)) as [
      ISectaDexPool,
      ISectaDexPool,
      ISectaDexPool
    ]
  ...
  let pools: [ISectaDexPool, ISectaDexPool, ISectaDexPool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestSectaDexCallee')
